...
class QSet:
    ...
    def GenerateSet(self,option):
        if (option == 1):
            ...
            self.querySet = _genOption2()
        elif (option == 3):
            self.querySet = _genOption3()
    ...
    def _genQuery1():
        ...
        for i in range(numSamplesGenerate):
            #Find the account
            featureVec = list()
            attribute = "actno"
            operation = "="
            value = rand.randint[1000,9999] #These are magic, but i figured its a healthy range for the RNG
    ...
    def _genQuery6():
        ...
        for i in range(numSamplesGenerate):
            ...
            attr = "bal"
            op = ">=" 
            value = rand.randint(0,1000000)#1million
